...
if PLATFORM == 'gcc':
    ...
    DEVICE = ARCH_ABI  + ' -DUSE_NONVECTOR_MODE=1  -DCONFIG_NDEBUG_CONSOLE=1 ' + ' -mtune=andes-d45-series  -Wall -Wno-undef -Wno-unused-variable -Wno-format -Wno-ignored-attributes -fno-builtin -ffunction-sections -fdata-sections  -Wno-implicit-function-declaration -Wno-incompatible-function-pointer-types '
    CFLAGS = DEVICE
    AFLAGS = CFLAGS
    if NO_OMIT_FRAME_POINTER:
        CFLAGS += ' -fno-omit-frame-pointer '
    ...
    ARCH_ABI = ' --target=riscv32-unknown-elf -march=' + RV_ARCH + ' -mabi=' + RV_ABI + ' '
    DEVICE = ARCH_ABI  + ' -DUSE_NONVECTOR_MODE=1  -DMAIN_ENTRY=entry  -DCONFIG_NDEBUG_CONSOLE=1 ' + ' -mtune=andes-d45-series -Wall -Wundef -Wno-format -Wno-ignored-attributes -fomit-frame-pointer -fno-builtin -ffunction-sections -fdata-sections  '
    CFLAGS = DEVICE
    AFLAGS = CFLAGS
    LFLAGS  = ARCH_ABI + ' -u _printf_float -u _scanf_float -static -nostartfiles -Wl,-Map=rtthread.map,--gc-sections,--orphan-handling=place,-print-memory-usage '
