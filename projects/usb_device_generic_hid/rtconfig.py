...
if PLATFORM == 'gcc':
    ...
    ARCH_ABI = ' --target=riscv32-unknown-elf -march=' + RV_ARCH + ' -mabi=' + RV_ABI + ' '
    DEVICE = ARCH_ABI  + ' -DUSE_NONVECTOR_MODE=1  -DMAIN_ENTRY=entry  -DCONFIG_USB_DCACHE_ENABLE=1 -DCONFIG_CHERRYUSB_CUSTOM_IRQ_HANDLER=1 -DCONFIG_NDEBUG_CONSOLE=1 ' + ' -mtune=andes-d45-series -Wall -Wundef -Wno-format -Wno-ignored-attributes -fomit-frame-pointer -fno-builtin -ffunction-sections -fdata-sections  '
    CFLAGS = DEVICE
    AFLAGS = CFLAGS
    LFLAGS  = ARCH_ABI + ' -u _printf_float -u _scanf_float -static -nostartfiles -Wl,-Map=rtthread.map,--gc-sections,--orphan-handling=place,-print-memory-usage '
