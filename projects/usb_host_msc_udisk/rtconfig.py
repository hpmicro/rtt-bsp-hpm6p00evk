...
if PLATFORM == 'gcc':
    ...
    CFLAGS = ARCH_ABI  + ' -DUSE_NONVECTOR_MODE=1  -DUSB_HOST_MCU_CORE=HPM_CORE0 -DCONFIG_NOT_ENABLE_DCACHE -DRT_DFS_ELM_USE_EXFAT=1 -D__USE_EXT_DISKIO -DUSB_FATFS_ENABLE=1 -DCONFIG_USB_DCACHE_ENABLE=1 -DCONFIG_CHERRYUSB_CUSTOM_IRQ_HANDLER=1 ' + ' -ffunction-sections -fdata-sections -fno-common '
    AFLAGS = CFLAGS
    ...
    CFLAGS = ARCH_ABI  + ' -DUSE_NONVECTOR_MODE=1  -DUSB_HOST_MCU_CORE=HPM_CORE0 -DCONFIG_NOT_ENABLE_DCACHE -DRT_DFS_ELM_USE_EXFAT=1 -D__USE_EXT_DISKIO -DUSB_FATFS_ENABLE=1 -DCONFIG_USB_DCACHE_ENABLE=1 -DCONFIG_CHERRYUSB_CUSTOM_IRQ_HANDLER=1 ' + ' -mtune=andes-d45-series  -Wall -Wno-undef -Wno-unused-variable -Wno-format -Wno-ignored-attributes -fomit-frame-pointer -fno-builtin -ffunction-sections -fdata-sections  -Wno-implicit-function-declaration'
    AFLAGS = CFLAGS
    ...
    ARCH_ABI = ' --target=riscv32-unknown-elf -march=' + RV_ARCH + ' -mabi=' + RV_ABI + ' '
    CFLAGS = ARCH_ABI  + ' -DUSE_NONVECTOR_MODE=1  -DMAIN_ENTRY=entry  -DUSB_HOST_MCU_CORE=HPM_CORE0 -DCONFIG_NOT_ENABLE_DCACHE -DRT_DFS_ELM_USE_EXFAT=1 -D__USE_EXT_DISKIO -DUSB_FATFS_ENABLE=1 -DCONFIG_USB_DCACHE_ENABLE=1 -DCONFIG_CHERRYUSB_CUSTOM_IRQ_HANDLER=1 ' + ' -mtune=andes-d45-series -Wall -Wundef -Wno-format -Wno-ignored-attributes -fomit-frame-pointer -fno-builtin -ffunction-sections -fdata-sections  '
    AFLAGS = CFLAGS
    LFLAGS  = ARCH_ABI + ' -u _printf_float -u _scanf_float -static -nostartfiles -Wl,-Map=rtthread.map,--gc-sections,--orphan-handling=place,-print-memory-usage '
